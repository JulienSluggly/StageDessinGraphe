...
void solve(Graphe& G) {
  ...
  edgeVector.push_back(make_pair(0,1));
  edgeVector.push_back(make_pair(2,3));
  std::vector<std::pair<int,int>> slotsVector;
  slotsVector.push_back(make_pair(0,0));
  slotsVector.push_back(make_pair(0,4));
  slotsVector.push_back(make_pair(5,0));
  slotsVector.push_back(make_pair(5,4));
  ...
  */
  ...
  for (int i = 0; i < nbNode; i++) {
    std::cout << "x[" << i << "] = " << cplex.getValue(nodes[i]) << std::endl;
  }
