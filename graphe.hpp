...
public:
	...
	// Attention cette variable n'est pas forc�ment � jour!
	...
	// Effectue 100 d�placements de recuit simul�! 
	void tempsCalculRecuitSimule(double cool = 0.99999, double t = 100, int mode = 0) {
		...
		for (int iter = 0; iter < 10000 && nbCroisement > 0; iter++) {
			int randomId;
			if (mode == 0)
				randomId = generateRand(_noeuds.size() - 1); // Selection al�atoire du noeud
			else if (mode == 1) {
			...
			int randomEmpId = generateRand(_emplacementsPossibles.size() - 1); // Selection al�atoire d'un emplacement disponible (pas tres �quiprobable)
	...
	int selectionEmplacementTournoiBinaire(int nodeId) {
		...
		int randomEmpId2 = generateRand(_emplacementsPossibles.size() - 1); // Selection al�atoire d'un emplacement disponible (pas tres �quiprobable)
		// on retire si on pioche le meme emplacement
	...
	int selectionEmplacement(int modeEmplacement, int nodeId, int t) {
		...
		if (modeEmplacement == 0) {
			slotId = generateRand(_emplacementsPossibles.size() - 1); // Selection al�atoire d'un emplacement disponible (pas tres �quiprobable)
			// on retire si on pioche le meme emplacement
	...
	// Lance l'algorithme de recuit simul� sur le graphe pour minimiser le nombre d'intersection
	// Met � jour la variable nombreCroisement du graphe.
	...
	// Lance l'algorithme de recuit simul� sur le graphe pour minimiser le nombre d'intersection
	// Utilise un delay pour rester plus longtemps � la meme temp�rature
	...
	// Lance l'algorithme de recuit simul� sur le graphe pour minimiser le nombre d'intersection
	// Tire 2 emplacements al�atoires et prend le plus proche
	...
	// Lance l'algorithme de recuit simul� sur le graphe pour minimiser le nombre d'intersection
	// Tire plusieurs emplacements al�atoires et prend le plus proche, augmente le tirage avec le temps
	...
	// Renvoie le nombre de noeud non associ� � un emplacement.
	int nbAPlace() {
		int nb = 0;
		for (Noeud noeud : _noeuds)
		{
			if (!noeud.estPlace()) nb++;
		}
		return nb;
	}
	...
	// Ajoute _noeud.size() emplacement par d�faut ou n emplacement 
	// Si le nombre d'emplacement est laiss� � -1, on g�nere dans une grille de taille _noeud.size()*_noeud.size()
	...
	// Ne met pas � jour le nombre de croisement d'un graphe
	bool croisementVoisinageFrom(Graphe& originalGraphe1, Graphe& originalGraphe2, bool useRand) {
		...
		while (nbNoeudATraiter > 0) {
			...
			//Si les lieux coincident les noeuds ne sont plus � traiter
	...
	// Nombre de noeuds du m�me ID plac� aux m�mes emplacements.
